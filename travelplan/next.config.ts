...
const nextConfig: NextConfig = {
  experimental: {
     ...
     *
     * **What this number covers now is the six file upload routes, and only them.** Story 2.34
     ...
     * `trips/[id]/day-plan-items/images` at 5 MB each, `trips/[id]/days/[dayId]/image` at 15 MB, and
     * — since Story 9.1 — `trips/[id]/accommodations/documents` and
     * `trips/[id]/day-plan-items/documents` at 10 MB each; every one of them calls
     * `request.formData()`, so a matched request costs Next's buffer plus the `File` it builds out of
     * it.
     *
     ...
     * routes is attacked. **The two document routes did not move it:** 10 MB arrives below the day
     * image route, so the highest per-route ceiling is still 15 MB and 20 MB still clears it. Raising
     * it is only correct if one of those six routes raises its own `MAX_FILE_SIZE_BYTES` past 15 MB
     * first.
     *
     * **Lowering it moved the truncation cliff, so every matched route grew a guard for it.** Over
     * this number Next truncates rather than refuses (`getCloneableBody` logs and pushes `null`),
     * which is how an intact file came to be reported as damaged in the first place. At 320 MB no
     * real upload ever reached the cliff; at 20 MB a 25 MB photo does, and it would have answered
     * `invalid_form_data` where 320 MB answered "exceeds size limit". Each of the six now checks
