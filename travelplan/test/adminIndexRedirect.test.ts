 ...
 *
 * `proxy.ts`'s matcher comment claimed "`/admin` today redirects to `/admin/users`" and nothing did -
 ...
 *
 * The matcher assertion in `proxy.test.ts` covers `/admin`, which *reinforced* the false claim rather
 * than catching it: being matched by the proxy only decides that a signed-out caller is redirected to
 * login, not that the path resolves to anything afterwards. So this asserts the page module itself.
 *
 * Its own file because the mock has to be module-scoped, and `proxy.test.ts` has no other reason to
