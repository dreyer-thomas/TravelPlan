...
export const POST = async (request: NextRequest, context: RouteContext) => {
  ...
  const uploadDir = getAccommodationImageUploadDir(tripId, parsed.data.tripDayId, parsed.data.accommodationId);
...
export const PATCH = async (request: NextRequest, context: RouteContext) => {
  ...
  // `reorderAccommodationImages` resolved through an owner-only scope. That scope is now the writer
  // clause, so without a gate here a viewer's refusal would come back from the repository as a 404 and
  // silently break AC6 on this one verb.
  const refusal = await refuseUnlessTripWriter(userId, tripId, "Accommodation not found");
  if (refusal) {
    return refusal;
  }
