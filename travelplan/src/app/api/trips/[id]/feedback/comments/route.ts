...
import { addTripFeedbackComment } from "@/lib/repositories/tripFeedbackRepo";
import { tripFeedbackCommentSchema } from "@/lib/validation/tripFeedbackSchemas";
...
export const POST = async (request: NextRequest, context: RouteContext) => {
  ...
  const parsed = tripFeedbackCommentSchema.safeParse(rawPayload);
  if (!parsed.success) {
  ...
  try {
    const feedback = await addTripFeedbackComment({
    ...
    return fail(apiError("server_error", "Unable to save feedback comment"), 500);
  }
};
