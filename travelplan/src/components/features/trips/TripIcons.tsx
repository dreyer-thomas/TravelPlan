...
export function WalkIcon({ sx }: IconProps) {
  return (
    <SvgIcon aria-hidden viewBox="0 0 24 24" sx={[{ fontSize: 14 }, ...(Array.isArray(sx) ? sx : [sx])]}>
      <circle cx="13" cy="4.5" r="2" fill="none" stroke="currentColor" strokeWidth={2} />
      <path
        d="M11.2 9 8 11.2 6.5 15"
        fill="none"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
...
export function BikeIcon({ sx }: IconProps) {
  return (
    <SvgIcon aria-hidden viewBox="0 0 24 24" sx={[{ fontSize: 14 }, ...(Array.isArray(sx) ? sx : [sx])]}>
      <circle cx="5.5" cy="16.5" r="3.5" fill="none" stroke="currentColor" strokeWidth={2} />
      <circle cx="18.5" cy="16.5" r="3.5" fill="none" stroke="currentColor" strokeWidth={2} />
      <path
        d="M5.5 16.5 10 8h4l4.5 8.5"
        fill="none"
        stroke="currentColor"
        strokeWidth={2}
        strokeLinecap="round"
      ...
      <path
        ...
        strokeLinejoin="round"
      />
      <path d="M8.5 5h2.5" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" />
    </SvgIcon>
  );
}

/**
 * The union is widened rather than backed by a default branch: the `never` assignment below turns a
 * newly added `transportType` member into a compile error here instead of letting it inherit the car
 * glyph silently. `CarIcon` after it is unreachable for any value the type admits and exists only so
 * a row that bypassed validation renders instead of crashing the timeline.
 */
export function transportIconFor(transportType: "car" | "ship" | "flight" | "walking" | "cycling") {
  ...
  const unhandled: never = transportType;
  void unhandled;
  return CarIcon;
}
