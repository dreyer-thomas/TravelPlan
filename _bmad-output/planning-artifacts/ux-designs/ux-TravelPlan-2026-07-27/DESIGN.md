...
- **candidate-list** — The list of places a location search came back with, under the place field on all four surfaces that have one (trip create — twice, activity dialog, accommodation dialog, bucket-list panel). A {typography.label-caps} heading naming the count ("Ort auswählen (5)"), then one full-width, left-aligned, {components.button.minHeight} row per candidate. **The label is the content** — the same rule `doc-chip` states: a place name is the only thing distinguishing "Sky Tower, Auckland" from "Sky Tower, Ho Chi Minh City", so the row carries the geocoder's full display name and nothing else.
  ...
  Rows are **rule-separated inside one group**, not individually bordered {rounded.md} cards: a `borderTop` on the group, a `borderBottom` per row, none on the last — the treatment `TripShareDialog`'s collaborator list already uses for a short list of same-kind rows. Cards would read as four things to compare; a ruled list reads as one question with four answers, which is what it is. An empty list draws nothing at all rather than a stray hairline.

  There is **no selected state**, and that is a property of the interaction rather than an omission: activating a row resolves the pin and dismisses the list in the same gesture, so a row could only be selected for the instant before it disappeared. The confirmation is the read-only `Latitude: … · Longitude: …` line below, which the field already had.

  Each row is a real button and therefore Tab-reachable with its candidate's name as its accessible name — deliberately not a `listbox`, whose arrow-key contract would be a new keyboard vocabulary for a list that exists for a few seconds at a time. The rows sit inside an explicit `role="group"` named by the heading, not a bare `<div>`: a `<div>`'s implicit `generic` role *prohibits* an accessible name, so without the role the heading's label is dropped and the group named here would not exist for the people it is for. The heading itself is a live region (`role="status"`, the convention `photo-viewer`'s counter already uses), because focus stays on *Find* when the list arrives and its count line is the only thing that announces a choice is now waiting. That heading is therefore **present and empty from the start**, not mounted along with the first result: a live region only announces a *change* in something already being watched, so a region that arrives carrying its own first text is routinely never read out — which would have left this the one announcement the pattern rests on and the one that never happened.
