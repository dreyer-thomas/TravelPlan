...
|---|---|---|
| Trip Overview (Screen A) | Trips List row tap; default landing after opening a trip | Whole-trip status at a glance: hero, top-line stats, day-by-day list with per-day coverage, and a sidebar of cost summary, route map, bucket list (owner only — a viewer and a contributor see cost summary, route map, gap alert), gap alert |
...
**Bucket List and Day-Map are embedded sidebar panels, not separate surfaces or routes — the Bucket List in both Trip Overview's (owner only) and Day Detail's sidebar, the Day-Map in Day Detail's only.** They correspond to real, already-existing product features (bucket list: epics.md Story 4.1/4.2; day map: Story 3.5) that were simply missing from earlier mockups in this pass and were added back as sidebar cards using existing card/map-preview conventions — no new navigation, no new route, no new visual language.
...
| "Reise behalten" beside "Reise löschen" — both buttons name an outcome, so the choice is a choice. | "Abbrechen" beside "Löschen" — asks the reader to work out *what* is being cancelled: the question, or the deletion? |
...
| button (primary / secondary) | Every dialog/form footer (Trip erstellen, Eintrag hinzufügen, Share Dialog, Passwort zurücksetzen) | Exactly one primary (committing) action per dialog. **A form dialog carries no cancel button** — it is dismissed by `icon-button.close` in the title row. A **destructive confirmation** keeps two buttons on equal footing, and the safe one names what it preserves ("Reise behalten"), never the mechanism ("Abbrechen") — see Voice and Tone. The earlier rule paired a secondary with every non-trivially-reversible primary; that held while the close affordance did not exist. The primary button is never disabled-with-no-explanation for a missing required field — the offending field shows its own error state instead (see State Patterns), so the reason a save can't proceed is always visible on the field, not inferred from a grayed-out button. The Share Dialog's `Entfernen` (remove collaborator) action is a `button` variant — sized to the same 44px hit area as every other button, not a bare unpadded text link — closing the touch-target gap `review-accessibility.md` flagged; it keeps its link-like (text, no fill) visual treatment. Visual spec: `DESIGN.md.Components → button`. |
| icon-button | Dialog title rows (`close`), day-hero (overflow, day navigation, day-image), activity dialog (delete) | A square 44×44 glyph control with **no visible label and a mandatory accessible name**, written per Voice and Tone ("Planpunkt löschen", not "Löschen"). Used where the action is either self-evident from position (a dialog's close) or already discoverable in text elsewhere; never as the only representation of something a user could not otherwise find. This system prefers words to glyphs — every icon-only control is a deliberate exception and carries its word in the accessible name. Visual spec: `DESIGN.md.Components → icon-button`. |
...
| Empty bucket list | Trip Overview sidebar `bucket-list` card; Day Detail sidebar `bucket-list` card | **Confirmed treatment (Tommy's decision, 2026-08-01; recorded in `_bmad-output/implementation-artifacts/7-12-bucket-list-sidebar-card.md`)** — no longer an assumption. Consistent with Voice and Tone: a short, concrete empty message (e.g. "Noch keine Ideen gesammelt.") plus a pointer to the add action, inside the existing `card` shell — no illustration, no separate empty-state component invented. The card also stays **compact**: no minimum height, no filler content, no reserved row space — it shrinks to the label, the count line and that one message. An empty bucket list must not read as a large blank card next to the day list. The same decision bounds the *expanded* card on **Trip Overview** — it grows to roughly 5–6 rows, then caps and scrolls internally (`md` and up only; below `md` the layout is one column and the cap is released so no scroll nests inside the page's own). Day Detail's card is a separate component and is **not** capped today — that surface was outside Story 7.12's scope, so treat an unbounded Day Detail bucket list as open work, not as this row's specified behaviour. |
...
| Dismissing a dialog with unsaved input | Any form dialog, via `icon-button.close` or Escape | **Ask once, and only if there is something to lose.** An untouched form closes silently. A form the user has typed into confirms first, naming what goes ("Deine Änderungen an diesem Planpunkt werden verworfen.") with the keeping answer as the safe one, per Voice and Tone. This exists because the dismissal moved from a labelled footer button to a 44px glyph in the corner: easier to hit by accident, and carrying no word that names the consequence. Without it, five minutes of typing sits behind an unguarded `✕`. The confirmation is the price of the smaller exit, not an optional refinement. |
