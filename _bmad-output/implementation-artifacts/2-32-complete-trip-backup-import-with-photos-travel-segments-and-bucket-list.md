...
- 2026-08-02: **Operator pass carried out, and it found a blocker.** Actions 2–5 verified against a throwaway copy of `dev.db` on port 3099 in a separate git worktree. Export: valid STORE ZIP, `application/zip`, `trip.json` + photos, readable by `unzip`. Round-trip: byte-identical copies under the imported trip's own ids; deleting the source removed its upload dir and left the copy's files and rows intact (0 orphan days/accommodations/plan items); a `formatVersion: 1` backup imported clean; overwrite replaced rather than duplicated (41 days not 82, 2 bucket items not 4). **Action 2 failed on the stock config**: `middleware.ts:66` matches `/api/trips/:path*`, so Next buffers the body and caps it at 10 MB — a 13.4 MB export was truncated and the route answered `400 invalid_form_data` for an intact backup. `importLimits.ts:14`'s premise ("nothing caps a request body unless the handler does") is wrong for Next 16 on a matched route. Fixed in `next.config.ts` via `experimental.proxyClientMaxBodySize: "110mb"` and re-proven with a 16.0 MB import: HTTP 200, 5 photos, no truncation. Nginx's `client_max_body_size` (operator action 1, done by Tommy) is necessary but not sufficient. One low-severity finding recorded as DW-95: a skipped photo's original URL is carried onto the imported trip, pointing at the source's directory. Status: done.
