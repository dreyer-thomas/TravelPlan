...
**AC5 satisfied on 2026-08-12, then deliberately superseded on 2026-08-15.** At the time this story landed, TravelPlan ran Node 24 and TravelBlogs was untouched on `/usr/bin/node` v20.19.6 — which is what AC5 required. Three days later the maintainer migrated TravelBlogs to Node 24 as well and **removed the Node 20 package**, so `/usr/bin/node` no longer exists. AC5 was met and then intentionally made obsolete; it was not violated. Two consequences: `DW-330` (TravelBlogs' own EOL exposure) is closed rather than merely decided, and **there is no rollback target on the host** — the deployment guide says so where it describes rollback. What the box turned out to be is recorded in [deployment-guide.md](../../docs/deployment-guide.md) and [deployment-configuration.md](../../docs/deployment-configuration.md) — systemd, units `TravelPlan.service` / `TravelBlogs.service`, `User=app` **shared by both**, trees under `/home/app/apps/`, host `Travelblog`, `arm64`, media root `/var/lib/travelplan/media`.
...
**Task 4's last gap closed on 2026-08-15.** The deploy script was shared and its behaviour is now written into the guide: a pre-flight dirty-tree refusal, a service stop with an `ERR` trap that restarts on any failure, a database backup, `git pull --ff-only`, a lockfile-hash-gated `npm ci`, `prisma migrate deploy`, `npm run build`, restart. Two corrections to the record followed from reading it: **migrations *are* applied automatically**, which the repository alone does not reveal; and **`DW-328` drops from high to low**, because `git pull --ff-only` updates the tree in place and never replaces the directory, so the data-loss trigger this story worried about does not exist in this deploy path.

**Three defects in that script were filed rather than fixed** — it is not in this repository, so it cannot be changed by a commit: `DW-332` (**high**) it backs up `prisma/dev.db` while the service runs `prisma/prod.db`, so ten generations of the wrong database are retained and **production has never been backed up**; `DW-333` the script is not under version control at all, which is why a defect of that size went unreviewed; `DW-334` it selects its build runtime implicitly and verifies with `systemctl is-active`, which is true of a process answering `500` to everything.

**Still uncaptured:** the browser click-through of a trip and a day view under Node 24, which needs credentials this session did not have. And `DW-332`'s one-command confirmation — `ls -la prisma/*.db` — which also determines whether `prisma migrate deploy` has been migrating a different database than the one the service reads.
...
| 2026-08-15 | Deploy script read; Task 4's last gap closed. `DW-328` lowered high → low (`git pull --ff-only` updates in place). Filed `DW-332` (backups target `dev.db`, production unbacked — high), `DW-333` (deploy script not in version control), `DW-334` (implicit build runtime, `is-active` used as a health check). |
| 2026-08-15 | TravelBlogs migrated to Node 24 by the maintainer and the Node 20 package removed. `DW-330` closed. AC5's arrangement superseded; guide, configuration doc and rollback section corrected — there is no rollback target on the host any more. |
